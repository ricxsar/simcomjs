...
SimCom.prototype.startGPSINFO = function(interval) {
  ...
  var getGPS = function(){
    if(self.connection && self.connection.isSending)
      return;
    self.execute('AT+CIND?')
    .then(function(indicators){      
      return [indicators, self.execute('AT+CGNSINF')];
    })
    .spread(function(indicators, res){
      var indicators = indicators.lines[res.lines.length-1].replace(/\s+/g, '').split(':')[1].trim().split(',').join(';');
      var s = res.lines[res.lines.length-1].replace(/\s+/g, '').split(':')[1].trim().split(',');
